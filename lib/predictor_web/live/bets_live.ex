defmodule PredictorWeb.BetsLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, assign(socket, page_title: "My Bets", bets: bets, bets_error: error, page: 1, last_odds_update: nil, summary: perf(bets))}
  end
  ...
  defp load_bets do
    bets = Repo.all(from b in Bet, where: b.status in ["accepted","placed","settled","pending"], order_by: [desc: b.placed_at, desc: b.id], preload: [fixture: [:home_team, :away_team], market: [], selection: [], bookmaker: []], limit: 50)
    {bets, nil}
  rescue e in [DBConnection.ConnectionError, Ecto.QueryError, Postgrex.Error] -> Logger.error("Bets load failed: #{Exception.message(e)}"); {[], "Please retry after the database connection recovers."}
  end
  defp fixture_name(nil), do: "Unknown fixture"
  defp fixture_name(f), do: "#{f.home_team.name} vs #{f.away_team.name}"
  defp perf(bets) do
    staked=Enum.reduce(bets, Decimal.new(0), &Decimal.add(&2,&1.stake||Decimal.new(0))); pl=Enum.reduce(bets, Decimal.new(0), &Decimal.add(&2,&1.profit_loss||Decimal.new(0))); open=Enum.filter(bets,&((&1.result in [nil,""]) and &1.status != "settled"))|>Enum.reduce(Decimal.new(0),&Decimal.add(&2,&1.stake||Decimal.new(0))); won=Enum.count(bets,&(&1.result=="won")); settled=Enum.count(bets,&(&1.result in ["won","lost","void"])); %{staked: staked, pl: pl, open: open, roi: if(Decimal.compare(staked,0)==:gt, do: percent(Decimal.mult(Decimal.div(pl,staked),100)), else: "—"), win_rate: if(settled>0, do: "#{round(won/settled*100)}%", else: "—"), settled: "#{settled}/#{length(bets)}"}
  end
end
