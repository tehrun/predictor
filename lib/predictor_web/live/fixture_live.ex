defmodule PredictorWeb.FixtureLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    {:ok, assign(socket, Map.merge(data, %{page_title: title, fixture_error: error, active_tab: "overview", last_odds_update: latest(data.odds_history)}))}
  end
  def handle_event("tab", %{"tab" => tab}, socket), do: {:noreply, assign(socket, :active_tab, tab)}
  ...
  attr :title, :string, required: true; slot :inner_block, required: true
  ...
  attr :rows, :list, required: true; attr :time_field, :atom, required: true
  ...
  defp load_all(id) do
    ...
    if is_nil(fixture), do: {%{fixture: nil, odds_history: [], fair_probabilities: [], recommendations: [], closing_lines: []}, "Fixture not found"}, else: {%{fixture: fixture, odds_history: q(OddsSnapshot,id,:captured_at,[:bookmaker,:market,:selection]), fair_probabilities: q(FairOdd,id,:calculated_at,[:market,:selection]), recommendations: q(ValueRecommendation,id,:recommended_at,[:bookmaker,:market,:selection]), closing_lines: q(ClosingLine,id,:captured_at,[:bookmaker,:market,:selection])}, nil}
  rescue e in [DBConnection.ConnectionError, Ecto.QueryError, Postgrex.Error, ArgumentError] -> Logger.error("Fixture load failed id=#{id}: #{Exception.message(e)}"); {%{fixture: nil, odds_history: [], fair_probabilities: [], recommendations: [], closing_lines: []}, "Fixture data could not be loaded."}
  end
  defp q(schema,id,field,preloads), do: Repo.all(from x in schema, where: x.fixture_id == ^id, order_by: [desc: field(x, ^field), desc: x.id], preload: ^preloads, limit: 100)
  defp fixture_name(f), do: "#{f.home_team.name} vs #{f.away_team.name}"
  defp latest([]), do: nil
  defp latest(rows), do: rows |> Enum.map(&Map.get(&1,:captured_at)) |> Enum.reject(&is_nil/1) |> Enum.max_by(&DateTime.to_unix/1, fn -> nil end)
  defp best([], _), do: nil
  defp best(rows, field), do: rows |> Enum.map(&Map.get(&1, field)) |> Enum.reject(&is_nil/1) |> Enum.max_by(&Decimal.to_float/1, fn -> nil end)
end
